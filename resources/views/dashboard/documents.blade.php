                    ...
                    <div id="sortable-files" class="flex flex-wrap justify-center md:justify-start gap-5">
                        @if (!empty($directories) || !empty($files))
                            ...
                            @foreach ($files as $index => $file)
                                <div class="file-card grid grid-cols-1 bg-white rounded-xl p-4 shadow-lg h-45 w-45 select-none cursor-pointer hover:bg-gray-50" data-id="{{ $file }}" id="file-{{ basename($file) }}" ondblclick="window.open('{{ Storage::url($file) }}', '_blank')" title="{{ basename($file) }}">
                                    <div class="flex items-start justify-between">
                                        <div class="bg-[#C8EAFF] rounded-full p-3">
...
<script>
    ...
    //     // Step 1: Check if the file already exists or if the folder exists
    ...
    //                 text: `"${relativePath}" already exists. Do you want to overwrite it?`,
    //                 icon: 'warning',
    ...
    //                     proceedUpload(file, relativePath);
    //                 }
    //             });
    //         } else {
    //             proceedUpload(file, relativePath);
