...
@section('title', 'File Manager | Media')
        ...
        <div class="container">
            <div class="p-1">
                <div id="drop-zone" class="relative m-5 p-8 bg-[#E8F8FF] rounded-xl min-h-150">
                    ...
                    <a href="{{ route('media') }}" class="font-bold text-3xl hover:underline mb-2">
                        Media
                    </a>
                    @if (!empty($folderExploded))
                        @foreach ($folderExploded as $index => $folderSegment)
                            ...
                            /
                            <a href="{{ route('media', ['folder' => implode('/', array_slice($folderExploded, 0, $index + 1))]) }}"
                    ...
                    <div id="sortable-files" class="flex flex-wrap justify-center md:justify-start gap-5">
                        @if (!empty($directories) || !empty($files))
                            @foreach ($directories as $directory)
                                <div class="folder-card grid-view grid grid-cols-1 content-between bg-white rounded-xl p-4 shadow-lg h-45 w-45 select-none cursor-pointer hover:bg-gray-50" data-id="{{ $directory }}" id="folder-{{ basename($directory) }}" ondblclick="window.location.href='{{ route('media', ['folder' => str_replace('public/user/'.auth()->user()->id.'/media/', '', $directory)]) }}'" title="{{ basename($directory) }}">
                                            ...
                                            <div class="flex justify-end">
                                                ...
                                                <div id="docuDropdown-{{ $directory }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow-sm">
                                                    <ul class="py-2 text-sm text-gray-700" aria-labelledby="docuButton-{{ $directory }}">
                                                        <li><a href="{{ route('media', ['folder' => str_replace('public/user/'.auth()->user()->id.'/media/', '', $directory)]) }}" class="block px-4 py-2 hover:bg-gray-100">Open</a></li>
                                                        <li>
                                                            <button 
                                                                class="block px-4 py-2 hover:bg-gray-100 bg-transparent border-0 cursor-pointer"
                                                                onclick="downloadFolder('{{ route('folders.download', ['folder' => str_replace('public/user/'.auth()->user()->id.'/media/', '', $directory)]) }}');">
                                                                Download
                                                            </button>
                                                        </li>
                                ...
                                {{-- List View --}}
                                <div class="folder-card list-view bg-white rounded-xl shadow-lg h-15 w-full select-none cursor-pointer hover:bg-gray-50 hidden" data-id="{{ $directory }}" id="folderList-{{ basename($directory) }}" ondblclick="window.location.href='{{ route('media', ['folder' => str_replace('public/user/'.auth()->user()->id.'/media/', '', $directory)]) }}'" title="{{ basename($directory) }}">
